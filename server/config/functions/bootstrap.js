...
module.exports = async (ctx) => {
  ...
  connection.once("ready", async () => {
    // 初始化 添加新文章队列
    const addArticleQueue = new Queue("AddArticle", { connection });
    // 添加新文章队列任务
    ...
    strapi.feeder.on("new-item", async (item) => {
      ...
      // 添加到新文章任务队列
      addArticleQueue.add(`${item.guid}_${item.meta.link}`, item);
    });
        ...
        if (article && article.website.rssUrl === item.meta.link) {
          ...
          if (website) {
            ...
            if (
              ...
              item.enclosures[0].type.indexOf("image") !== -1
            ) {
